...
5. **Sexual Orientation**
   - "No Preference" matches all ✅
   - Exact orientation match (Straight wants Straight) ✅
   - Orientation mismatch (Straight wants Gay, candidate is Straight) ❌
...
### `emulatorSetup.ts`
**Purpose:** Helper utilities for emulator-based testing

**Functions:**
- `setupEmulatorTests()` - Initialize Firebase app and connect to emulators
- `cleanupEmulatorTests()` - Clean up after tests
- `getTestAuth()`, `getTestFunctions()`, `getTestFirestore()` - Get Firebase service instances
- `areEmulatorsRunning()` - Check if emulators are available
- `clearFirestoreEmulator()` - Reset emulator state between tests
- `seedTestData()` - Populate emulator with test data
- `waitForEmulator()` - Wait for emulator to be ready (CI/CD)
...
npm test src/__tests__/integration/itineraryMatching.integration.test.ts
...
3. Update `emulatorSetup.ts` if new Firebase services are needed
